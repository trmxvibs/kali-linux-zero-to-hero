...
**Fully written modules:** 00–13, 21, 22, 26 — i.e. the complete **Linux
Fundamentals** track (00–07), **Networking** (08–09), **Security
Fundamentals** (10), and **Reconnaissance, Enumeration & Traffic Analysis**
(11–13), plus Automation and Lab-Building. See `README.md` for the
...
commands, Module 09's `ip`/`ping`/`dig` commands, Module 12's
`smbclient`/`enum4linux`/`showmount`/NSE-script commands, and Module 13's
`tcpdump`/Wireshark/`tshark` commands (no systemd, network stack, packet
capture tools, or Metasploitable VM in the build sandbox) — all flagged
clearly in their own lessons and in `tests/TEST_LOG.md`.
...
1. Complete Modules 14–20 (the remaining hands-on Kali tool modules),
...
3. Verify Module 07's systemd/cron commands, Module 09's ip/ping/dig
   commands, Module 12's SMB/NFS/NSE commands, and Module 13's
   tcpdump/Wireshark commands against a real Kali VM + Metasploitable lab,
   and update `tests/TEST_LOG.md` from "documentation-verified" to
   "live-tested" once done.
